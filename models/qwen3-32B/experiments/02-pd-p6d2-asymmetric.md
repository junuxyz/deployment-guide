...
This allocation experiment biases the same round-robin P/D baseline toward
prompt processing: six TP=2 prefill replicas and two TP=2 decode replicas.
...
| Prefill pool A | `gpu05` | 4 | 2 | SGLang prefill |
| Prefill pool B | `gpu06` | 2 | 2 | SGLang prefill |
| Decode | `gpu06` | 2 | 2 | SGLang decode |
...
This 6P/2D layout dedicates all eight GPUs on `gpu05` to four prefill replicas,
while `gpu06` splits its eight GPUs between two prefill and two decode replicas.
...
`/ephemeral/shared/qwen3-32b/manifests/02-pd-p6d2-asymmetric.yaml`:

**Run on `gpu05` — Kubernetes admin terminal:** create the deployment file on
the shared filesystem.

```bash
tee /ephemeral/shared/qwen3-32b/manifests/02-pd-p6d2-asymmetric.yaml >/dev/null <<'EOF'
apiVersion: nvidia.com/v1beta1
kind: DynamoGraphDeployment
metadata:
  name: qwen32-pd-p6d2-asym
...
```bash
kubectl apply -f /ephemeral/shared/qwen3-32b/manifests/02-pd-p6d2-asymmetric.yaml
kubectl get dynamographdeployment qwen32-pd-p6d2-asym -n "$NAMESPACE" -w
```
...
kubectl get pods -n "$NAMESPACE" \
  -l nvidia.com/dynamo-graph-deployment-name=qwen32-pd-p6d2-asym \
  -o wide -w
```
...
Exactly four prefill pods must be on `gpu05`; two prefill and two decode pods
must be on `gpu06`; the frontend must be on `gpu05`:
...
kubectl logs -n "$NAMESPACE" \
  -l 'nvidia.com/dynamo-graph-deployment-name=qwen32-pd-p6d2-asym,nvidia.com/dynamo-component-type=prefill' \
  --all-containers --prefix --tail=2000 --max-log-requests=8 \
  | grep -Ei 'nixl|ucx|rdma|transport'

kubectl logs -n "$NAMESPACE" \
  -l 'nvidia.com/dynamo-graph-deployment-name=qwen32-pd-p6d2-asym,nvidia.com/dynamo-component-type=decode' \
  --all-containers --prefix --tail=2000 --max-log-requests=8 \
  | grep -Ei 'nixl|ucx|rdma|transport'
```
...
export FRONTEND_POD="$(kubectl get pods -n "$NAMESPACE" \
  -l 'nvidia.com/dynamo-graph-deployment-name=qwen32-pd-p6d2-asym,nvidia.com/dynamo-component-type=frontend' \
...
- placement is 4P on `gpu05`, 2P + 2D on `gpu06`, and frontend on `gpu05`;
...
```bash
kubectl delete dynamographdeployment qwen32-pd-p6d2-asym -n "$NAMESPACE" --wait=true
